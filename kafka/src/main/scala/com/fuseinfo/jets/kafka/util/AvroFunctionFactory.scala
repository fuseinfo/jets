...
object AvroFunctionFactory {
  private val IMPORTS = Array("com.fuseinfo.jets.util._","com.fuseinfo.jets.kafka.util._","org.apache.avro.generic._",
    "scala.concurrent._", "ExecutionContext.Implicits.global").map("import " + _ + "\n").mkString
...
"""
  ...
  def getFuncClass[T](keySchema:Schema, valueSchema:Schema, outSchema:Schema, mapping:ObjectNode, timeout:Long,
  ...
  private def scalaRecordGen(keySchema:Schema, valueSchema:Schema, outSchema:Schema, mapping:ObjectNode,
                             schemaName:String, timeout:Long, isRoot:Boolean = false,
                             funcType:String = "new ((GenericRecord, GenericRecord) => GenericRecord)",
    ...
    var hasFuture = false
    val mapDef = mapping.fields.filterNot(_.getKey.startsWith("__")).map{entry =>
      val entryKey = entry.getKey.trim
      val (fName, bg) =
      ...
      val setStat = entry.getValue match {
        case childObj:ObjectNode =>
          ...
          varSb.append(s"  val ${fName}_func = " +
            scalaRecordGen(keySchema, valueSchema, childSchema, childObj, childSchemaName, timeout))
          s"_recBuilder.set(_fieldArr($outIndex), ${fName}_func(_keyRecord,_valueRecord))"
        case textNode:ValueNode =>
          val text = VarUtils.enrichString(textNode.asText, KafkaFlowBuilder.vars) match {
            case "" => "\"\""
    ...
    val initDef = mapping.get("__init__") match {
      case null => ""
      case initNode => VarUtils.enrichString(initNode.asText, KafkaFlowBuilder.vars)
    }
    val foreachDef = mapping.get("__foreach__") match {
      case null => ""
      case foreachNode => VarUtils.enrichString(foreachNode.asText, KafkaFlowBuilder.vars)
    }
